...
namespace CognitiveServices.Explorer.Application
{
    public class HttpRequestService2
    {
        public async Task<string?> Send(HttpRequest request, CognitiveServiceConfig cognitiveServiceConfig, CancellationToken token = default)
        {
            ...
            {
                BaseAddress = new Uri(cognitiveServiceConfig.BaseUrl)
            };

            client.DefaultRequestHeaders.Add(request.TokenHeaderName, cognitiveServiceConfig.Token);
            ...
            {
                case "GET":
                    responseTask = client.GetAsync(request.RelativePath, token);
                    break;
                //case "POST":
                //    {
